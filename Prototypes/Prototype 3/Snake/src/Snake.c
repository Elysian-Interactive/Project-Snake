...
void Snake_move(int SCREEN_WIDTH, int SCREEN_HEIGHT)
{
	...
	if( ( ((SnakeNode*)DList_first(Snake))->body.x < 0 ) || ( ((SnakeNode*)DList_first(Snake))->body.x > SCREEN_WIDTH ))
		printf("Collision on x-axis");
	if( ( ((SnakeNode*)DList_first(Snake))->body.y < 0 ) || ( ((SnakeNode*)DList_first(Snake))->body.y > SCREEN_HEIGHT ))
